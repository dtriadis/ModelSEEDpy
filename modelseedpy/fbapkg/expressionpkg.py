...
class ExpressionPkg(BaseFBAPkg):
    ...
    def maximize_functionality(self, rxnIDs):
        ...
        self.util.add_objective(
            sum(
                [
                    self.msdb.reactions.get_by_id(rxnID).flux_expression
                    for rxnID in rxnIDs
                ]
            )
        )
    ...
    def build_gimme(
        ...
        max_req_funcs = {
            list(rxnIDs): self.maximize_functionality(rxnIDs)
            for rxnIDs in required_functionalities
        }
        ...
        self.coefs = {
            r_id: threshold - val for r_id, val in ex_data.items() if val < threshold
        }
        objective_coefs = {}
        for rxn in self.util.model.reactions:
            ...
            objective_coefs[rxn.forward_variable] = objective_coefs[
                rxn.reverse_variable
            ] = self.coefs[rxn.id]
        # define the objective expression
        self.util.add_objective(
            sum(list(objective_coefs.keys())), "min", objective_coefs
        )
