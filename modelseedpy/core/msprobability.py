...
class MSProbability:
    ...
    @staticmethod
    def megaModel(
        clades_paths, kbase_api=None, reaction_counts_path=None, numTotal="numMembers"
    ):
        ...
        for clade, paths in clades_paths.items():
            ...
            for model_path in paths:
                ...
                try:
                    ...
                    broken_models.append(model_path)
                    continue
                captured_reactions.extend(
                    [rxn for rxn in model.reactions if rxn.id not in captured_rxnIDs]
                )
                captured_rxnIDs.update([rxn.id for rxn in model.reactions])
                remaining_rxnIDs -= captured_rxnIDs
                if remaining_rxnIDs == set():
                    break
            ...
            export_name = path.join(
                reaction_counts_path, f"{clade}.xml"
            )  # if re.search(f"[0-9]+\/[0-9]+\/[0-9]+", model_path) else f"{path.join(path.dirname(model_path), clade)}.xml"
