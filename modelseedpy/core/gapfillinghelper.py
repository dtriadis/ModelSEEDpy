...
from cobra.core import Gene, Metabolite, Model, Reaction  # !!! Gene is never used
...
class GapfillingHelper():
        ...
        with model:
            for rxn in reactions:
                ...
                for test in tests:
                    testmodel = model
                    with testmodel:
                        self.apply_media_to_model(testmodel,test["media"],test["default_uptake"],test["default_excretion"])  #!!! this function is undefined
                        self.set_objective_from_target_reaction(testmodel,test["target"],test["maximize"])  #!!! this function is undefined
                        solution = testmodel.optimize()  #!!! solution is unused
                        if test.maximize == 1:
    ...
    def convert_modelcompound(self, metabolite, bigg=False):
        met_id = build_id(metabolite.id)
        if bigg and "bigg.metabolite" in metabolite.annotation:
            met_id = metabolite.annotation["bigg.metabolite"]+"_"+metabolite.compartment
    ...
    def convert_cobra_compound_to_kbcompound(self, cpd, kbmodel=None, add_to_model=True):
        refid = "cpd00000"
    ...
    def convert_cobra_reaction_to_kbreaction(self, rxn, kbmodel, direction="=", add_to_model=True):
        ...
        for cpd in rxn.metabolites:
            if cpd.id not in kbmodel.modelcompounds:
                self.convert_cobra_compound_to_kbcompound(cpd,kbmodel)
    ...
    def compute_reaction_scores(self, weigh_all_events_equally=True, weights=None):
        reaction_genes = {}
