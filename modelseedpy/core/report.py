...
equation_format = lambda rxn: round_float_str(
...
def atp_summary_formatter(model):
    ...
    if "atp_c" in model.metabolites:
        ...
        # Empty ATP summary
        msg = (
            "None of the < atp_c, ATP_c, and cpd00002_c0 > metabolites are defined. "
            "Add one of these metabolites in the model to display an ATP summary."
        )
        return msg, False

    rxns = [
        (model.reactions.get_by_id(rxnID), rxnID)
...
def fba_report(
    ...
    model_id,
    media_id,
    export_html_path="steadycom_report.html",
):
    ...
    html_report = env.get_template(os.path.join("core", "fba_template.html")).render(
        content
    )
...
def steadycom_report(flux_df, exMets_df, export_html_path="steadycom_report.html"):
    ...
    content = {
        "flux_table": flux_df.style.background_gradient().to_html(),
...
sugars, aminoacids = load(os.path.join(categories_dir, "sugars.npy")), load(
    os.path.join(categories_dir, "aminoAcids.npy")
)
vitamins, minerals = load(os.path.join(categories_dir, "vitamins.npy")), load(
    os.path.join(categories_dir, "minerals.npy")
)
energy_compounds = load(os.path.join(categories_dir, "energy_compounds.npy"))
sugars_dic, aminoacids_dic = dict(zip(sugars[:, 0], sugars[:, 1])), dict(
    zip(aminoacids[:, 0], aminoacids[:, 1])
)
vitamins_dic, minerals_dic = dict(zip(vitamins[:, 0], vitamins[:, 1])), dict(
    zip(minerals[:, 0], minerals[:, 1])
)
...
def msdb_justification_report(
    proposed_changes, export_html_path="msdb_correction_report.html"
):
    ...
    content = {
        "table": proposed_changes_df,
        "heatmap": heatmap_changes.style.background_gradient().to_html(),
    }
    env = jinja2.Environment(
        loader=jinja2.FileSystemLoader(os.path.join(package_dir, "biochem")),
        autoescape=jinja2.select_autoescape(["html", "xml"]),
    )
